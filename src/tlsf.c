...
// Necessary for use of CHERI function
...
{
	return tlsf_cast(block_header_t*, __builtin_cheri_address_set(ptr, tlsf_cast(tlsfptr_t, ptr)+ size));
}
...
{
	const void* aligned = __builtin_align_up(ptr, ~__builtin_cheri_representable_alignment_mask(align) + 1 );
	tlsf_assert(0 == (align & (align - 1)) && "must align to a power of two");
	return tlsf_cast(void*, (uintptr_t)aligned);
}
